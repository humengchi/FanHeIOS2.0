...
- (void)_initialisation {
    ...
    _enableClickToDismiss = YES;
    _enableLongClickToShowBar = YES;
...
- (void)viewDidLoad {
    ...
    backBtn.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:16];
 
    [_toolbar addSubview:backBtn];
    
    UIButton *backBtn1 = [UIButton buttonWithType:UIButtonTypeCustom];
    [backBtn1 addTarget:self action:@selector(actionButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    backBtn1.frame = CGRectMake(WIDTH - 56, 0, 44, 44);
    [backBtn1 setTitle:@"保存" forState:UIControlStateNormal];
    backBtn1.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:16];
    
    [_toolbar addSubview:backBtn1];
...
//    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
//    }
    ...
    // Middle - Nav
    if (_previousButton && _nextButton && numberOfPhotos > 0) {//胡大爷改的>1
        hasItems = YES;
...
- (void)toggleControls {
     [self dismissViewControllerAnimated:YES completion:nil];
      [self.navigationController popViewControllerAnimated:YES];
    [self setControlsHidden:![self areControlsHidden] animated:NO permanent:NO];
}
...
//        UIImageView *images= [[UIImageView alloc]initWithFrame:CGRectMake(0, 10, 10, 10)];
//        images.image = image;
...
//            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"4")) {
