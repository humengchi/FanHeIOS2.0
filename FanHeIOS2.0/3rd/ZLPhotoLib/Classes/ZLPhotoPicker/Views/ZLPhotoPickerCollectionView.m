    ...
    if (self.isRecoderSelectPicker){
        ...
        for (ZLPhotoAssets *asset in self.selectAssets) {
            for (ZLPhotoAssets *asset2 in self.dataArray) {
                ...
                if (IOS_X < 8.0f) {
                    if ([asset.alAsset.defaultRepresentation.url isEqual:asset2.alAsset.defaultRepresentation.url]) {
                        [selectAssets addObject:asset2];
                        break;
                    }
                }else {
                    if ([asset.phAsset.localIdentifier isEqualToString:asset2.phAsset.localIdentifier]) {
                        [selectAssets addObject:asset2];
                        break;
                    }
                }
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        if (self.isRecoderSelectPicker) {
            for (ZLPhotoAssets *asset in self.selectAssets) {
                if (IOS_X<8.0f) {
                    if ([asset isKindOfClass:[ZLPhotoAssets class]] && [asset.alAsset.defaultRepresentation.url isEqual:[(ZLPhotoAssets *)self.dataArray[indexPath.item] alAsset].defaultRepresentation.url]) {
                        [self.selectsIndexPath addObject:@(indexPath.row)];
                    }
                }else {
                    if ([asset isKindOfClass:[ZLPhotoAssets class]] && [asset.phAsset.localIdentifier isEqualToString:[(ZLPhotoAssets *)self.dataArray[indexPath.item] phAsset].localIdentifier]) {
                        [self.selectsIndexPath addObject:@(indexPath.row)];
                    }
                }
            }
            ...
            if (IOS_X < 8.0f) {
                ...
                [[PHImageManager defaultManager] requestImageForAsset:asset.phAsset
                                                        ...
                                                        resultHandler:^(UIImage *result, NSDictionary *info) {
                                                            asset.originImage = result;
                                                            cellImgView.image = result;
                                                        }];
                /*CGFloat scale = [UIScreen mainScreen].scale;
                 CGFloat width = [UIScreen mainScreen].bounds.size.width;
                 CGFloat height = [UIScreen mainScreen].bounds.size.height;
                 CGSize targetSize = CGSizeMake(scale*width, scale*height);
                 [[PHImageManager defaultManager] requestImageForAsset:asset.phAsset targetSize:targetSize contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage *result, NSDictionary *info) {
                 if(result){
                 asset.originImage = result;
                 }
                 }];*/
