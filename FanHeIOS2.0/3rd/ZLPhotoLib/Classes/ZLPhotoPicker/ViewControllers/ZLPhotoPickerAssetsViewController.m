...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setTitleColor:kDefaultColor forState:UIControlStateNormal];
...
- (ZLPhotoPickerCollectionView *)collectionView{
    if (!_collectionView) {
        ...
        layout.footerReferenceSize = CGSizeMake(WIDTH, TOOLBAR_HEIGHT * 2);
        
        AssetGridThumbnailSize = CGSizeMake(cellW*[UIScreen mainScreen].scale, cellW*[UIScreen mainScreen].scale);
        ...
        collectionView.assetGridThumbnailSize = AssetGridThumbnailSize;
        // 时间置顶
    ...
    [[ZLPhotoPickerDatas defaultPicker] getGroupPhotosWithGroup:self.assetsGroup finished:^(NSArray *assets) {
        if (IOS_X<8.0f) {
            [assets enumerateObjectsUsingBlock:^(ALAsset *asset, NSUInteger idx, BOOL *stop) {
                ...
                zlAsset.alAsset = asset;
                [assetsM addObject:zlAsset];
            }];
        }else {
            [assets enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL *stop) {
                __block ZLPhotoAssets *zlAsset = [[ZLPhotoAssets alloc] init];
                zlAsset.phAsset = asset;
                [assetsM addObject:zlAsset];
            }];
        }
...
- (void) pickerCollectionViewDidSelected:(ZLPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(ZLPhotoAssets *)deleteAssets{
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            ...
            if (IOS_X < 8.0f) {
                if([[[[asset.alAsset defaultRepresentation] url] absoluteString] isEqualToString:[[[photoAsset.alAsset defaultRepresentation] url] absoluteString]]){
                    selectAssetsCurrentPage = i;
                    break;
                }
            }else {
                if ([asset.phAsset.localIdentifier isEqualToString:photoAsset.phAsset.localIdentifier]) {
                    selectAssetsCurrentPage = i;
                    break;
                }
            }
    ...
    if (self.selectAssets.count > indexPath.item) {
        ...
        if ([self.selectAssets[indexPath.item] isKindOfClass:[ZLPhotoAssets class]]) {
            if (IOS_X<8.0f) {
                ...
                ZLPhotoAssets *zlAsset = self.selectAssets[indexPath.item];
                [[PHImageManager defaultManager] requestImageForAsset:zlAsset.phAsset
                                                           targetSize:AssetGridThumbnailSize
                                                          contentMode:PHImageContentModeAspectFill
                                                              options:nil
                                                        resultHandler:^(UIImage *result, NSDictionary *info) {
                                                            //zlAsset.originImage = result;
                ...
                CGSize targetSize = CGSizeMake(scale*width, scale*height);
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [[PHImageManager defaultManager] requestImageForAsset:zlAsset.phAsset targetSize:targetSize contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage *result, NSDictionary *info) {
                        if(result){
                            zlAsset.originImage = result;
                        }
                    }];
...
- (void)photoBrowser:(ZLPhotoPickerBrowserViewController *)photoBrowser removePhotoAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for (NSInteger i = 0; i < self.collectionView.dataArray.count; i++) {
        ...
        if ([photoAsset isKindOfClass:[ZLPhotoAssets class]] && [asset isKindOfClass:[ZLPhotoAssets class]]) {
            ...
            if (IOS_X < 8.0f) {
                if([[[[assets.alAsset defaultRepresentation] url] absoluteString] isEqualToString:[[[photoAssets.alAsset defaultRepresentation] url] absoluteString]]){
                    currentPage = i;
                    break;
                }
            }else {
                if ([assets.phAsset.localIdentifier isEqualToString:photoAssets.phAsset.localIdentifier]) {
                    currentPage = i;
                    break;
                }
            }
