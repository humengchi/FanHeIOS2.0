...
@interface ZLPhotoPickerBrowserPhotoScrollView ()<PHPhotoLibraryChangeObserver,UIActionSheetDelegate> {
...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:photo.asset.phAsset targetSize:targetSize contentMode:PHImageContentModeAspectFit options:options resultHandler:^(UIImage *result, NSDictionary *info) {
            if (result) {
                [self setProgress:1.0f];
            }
            self.isLoadingDone = YES;
            if (result) {
                photo.asset.originImage = result;
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
    dispatch_async(dispatch_get_main_queue(), ^{
        PHObjectChangeDetails *changeDetails = [changeInstance changeDetailsForObject:self.photo.asset.phAsset];
        if (changeDetails == nil) {
            return;
        }
        self.photo.asset.phAsset = [changeDetails objectAfterChanges];
    });
}
