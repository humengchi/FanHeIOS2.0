...
- (void)showToastMessage:(NSString*)message{
    CGFloat width = [NSHelper widthOfString:message font:FONT_SYSTEM_SIZE(15) height:40]+40;
    CGFloat height = 40;
    if(width>250){
        width = 250;
        height = [NSHelper heightOfString:message font:FONT_SYSTEM_SIZE(15) width:250]+25;
    }
    UILabel *toastLabel = [UILabel createrLabelframe:CGRectMake((WIDTH-width)/2, (HEIGHT-height)/2, width, height) backColor:[HEX_COLOR(@"41464e") colorWithAlphaComponent:0.8] textColor:WHITE_COLOR test:message font:15 number:1 nstextLocat:NSTextAlignmentCenter];
    [CALayer updateControlLayer:toastLabel.layer radius:4 borderWidth:0 borderColor:nil];
...
- (void)showToast:(UIView *)view {
    [self showToast:view duration:MessageToastAutoDismissDuration position:@"navigation"];
}
...
- (UIView *)viewForMessage:(NSString *)message {
    ...
    if (message != nil) {
        ...
        messageLabel.font = FONT_SYSTEM_SIZE(12);
        ...
        NSDictionary *attribute = @{NSFontAttributeName: FONT_SYSTEM_SIZE(12)};
