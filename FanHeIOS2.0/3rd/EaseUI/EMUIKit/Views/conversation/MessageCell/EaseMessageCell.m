...
@property (nonatomic, assign)CGFloat voiceWideth;
                         ...
                         model:(id<IMessageModel>)model
{
    ...
    [_statusButton setImage:[UIImage imageNamed:@"icon_connection_sendfailed"] forState:UIControlStateNormal];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            ...
            case EMMessageBodyTypeImage:
            {
                __block id<IMessageModel> tmpModel = model;
                __block EaseBubbleView *tmpBubbleView = _bubbleView;
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    BOOL imageCash = YES;
                    UIImage *originImage = tmpModel.thumbnailImage;
                    CGSize retSize = tmpModel.thumbnailImageSize;
                    if(tmpModel.thumbnailImage){
                        originImage = tmpModel.thumbnailImage;
                        retSize = tmpModel.thumbnailImageSize;
                    }else if (tmpModel.image) {
                       originImage  = tmpModel.image;
                        retSize = tmpModel.imageSize;
                    }else{
                        originImage = KHeadImageDefault;
                    ...
                    if(retSize.width > retSize.height){
                        if(retSize.width/retSize.height > 2){
                            ...
                            sizeWidth = sizeHeight*retSize.width/retSize.height;
                        }else{
                        ...
                        if(retSize.height/retSize.width > 2){
                            ...
                            sizeHeight = sizeWidth*retSize.height/retSize.width;
                        }else{
                    ...
                    CGImageRef imageRef = CGImageCreateWithImageInRect([originImage CGImage], CGRectMake(0,0, sizeWidth, sizeHeight));//获取图片整体部分
                   __block UIImage *image = [UIImage imageWithCGImage:imageRef scale:originImage.scale orientation:UIImageOrientationUp];
                
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (imageCash==NO) {
                            [tmpBubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:tmpModel.fileURLPath] placeholderImage:[UIImage imageNamed:tmpModel.failImageName]];
                        } else {
                            tmpBubbleView.imageView.image = image;
                        }
                    });
                });
            }
...
//                    }
                    self.bubbleView.voiceImageView.image = self.model.isSender ? [UIImage imageNamed:@"icon_chat_re_playing003"] : [UIImage imageNamed:@"chat_receiver_audio_playing003"];
                    _bubbleView.voiceImageView.animationImages = self.model.isSender ? self.sendMessageVoiceAnimationImages:self.recvMessageVoiceAnimationImages;
                               ...
                               if (!self.model.isSender) {
                                   _bubbleView.voiceLabel.textAlignment = NSTextAlignmentLeft;
                    if (self.model.isMediaPlayed){
                        ...
                        _bubbleView.isReadView.hidden = NO;
                                  ...
                                  _bubbleView.voiceLabel.textAlignment = NSTextAlignmentRight;
                               }
               ...
               NSString *str = @"";
        for (NSInteger i = 0; i<(int)_model.mediaDuration; i++) {
            str = [NSString stringWithFormat:@"%@ ",str];
                    }
                
                _bubbleView.voiceDurationLabel.text = [NSString stringWithFormat:@"%d''%@",(int)_model.mediaDuration,str];
                 _bubbleView.voiceLabel.text = @"58''";
           _bubbleView.voiceLabel.text = [NSString stringWithFormat:@"%d''",(int)_model.mediaDuration];
            }
                break;
            case EMMessageBodyTypeVideo:
            {
...
//    if ([dic[@"money_type_special"] isEqualToString:@"dynamic"]) {
//        [_bubbleView updateDayImageMargin:_bubbleMargin];
//    }else
...
- (void)bubbleViewTapAction:(UITapGestureRecognizer *)tapRecognizer{
    NSDictionary *ext = _model.message.ext;
    if ([[CommonMethod paramNumberIsNull:ext[@"topage"]] integerValue]) {
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:
        {
            ...
            height += (rect.size.height > 15 ? rect.size.height : 15) + 5;
            height = height - 5;
...
//            CGSize retSize = model.imageSize.width?model.imageSize:model.thumbnailImageSize;
//            retSize = retSize.width>model.image.size.width?retSize:model.image.size;
            
            id<IMessageModel> tmpModel = model;
            CGSize retSize = tmpModel.thumbnailImageSize;
            if(tmpModel.thumbnailImage){
                retSize = tmpModel.thumbnailImageSize;
            }else if (tmpModel.image) {
        ...
        case EMMessageBodyTypeFile:
        {
            ...
            height += (nameRect.size.height > 15 ? nameRect.size.height : 15);
...
- (UIImage*)clipImageWithImage:(UIImage*)image inRect:(CGRect)rect {
    ...
    UIImage* clipImage = [UIImage imageWithCGImage:imageRef scale:1.0 orientation:UIImageOrientationUp];
