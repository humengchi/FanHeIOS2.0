...
@property (nonatomic, strong) UIView *backCoverShadow;     //底部视图
...
@property (nonatomic, assign) BOOL isSelectIndoir;

//点击菜单
@property (nonatomic, assign) BOOL isMenu;

@property (nonatomic, strong) TagSearchView *tagView;
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //         CGFloat wdteth = [NSHelper widthOfString:titleString font:[UIFont systemFontOfSize:14.0] height:14];
        
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        
        if (i == 2) {
            btn.frame = CGRectMake((i + 1)*separatorLineInterval - 14, self.frame.size.height / 2 - 3, 10, 10);
        }else{
            btn.frame = CGRectMake((i + 1)*separatorLineInterval - textLayerInterval + 16, self.frame.size.height / 2-3, 10, 10);
        }
        btn.tag = i+ 100;
        [btn setImage:kImageWithName(@"icon_barrow_grey") forState:UIControlStateNormal];
        [btn setImage:kImageWithName(@"icon_tarrow_red") forState:UIControlStateSelected];
        [self addSubview:btn];
        
        
        if (i == 3) {
            self.isSelectIndoir = YES;
        }else{
            self.isSelectIndoir = NO;
        }
...
-(void)setShow:(BOOL)show{
    ...
    if (!show) {
        if (self.currentIndexPath!=nil) {
            if (self.finishedBlock && !self.isMenu) {
                self.finishedBlock(self.currentIndexPath);
            }
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:ACTIVITYDOWNMENU object:nil];
        NSLog(@"收回");
    }
    self.isMenu = NO;
}
#pragma mark - init method
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height coverHeigth:(CGFloat)coverHeigth {
    ...
    if (self) {
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(0,self.menHeigth, WIDTH,HEIGHT*100)];
        _backGroundView.backgroundColor = CoverViewColor;
        _backGroundView.opaque = NO;
        UIGestureRecognizer *gesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(backgroundTapped:)];
        [_backGroundView addGestureRecognizer:gesture];
        //         [_buttomImageView addGestureRecognizer:gesture];
...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    //    layer.strokeColor = [UIColor colorWithHexString:@"AFB6C1"].CGColor;
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"menuButtons" object:nil];
    if (_dataSource == nil) {
        return;
    }
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            view.backgroundColor = CoverViewColor;
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        //        _backGroundView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, kButtomImageViewHeight);
        //           [[UIApplication sharedApplication].keyWindow addSubview:_buttomImageView];
        ...
        UIButton *btn = [self viewWithTag:self.currentIndex + 100];
        btn.selected = YES;
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            //            _backGroundView.frame = CGRectMake(self.origin.x, CGRectGetMaxY(_leftTableView.frame)-2, self.frame.size.width, kButtomImageViewHeight + HEIGHT);
        }];
    } else {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            //            _backGroundView.frame = CGRectMake(self.origin.x, CGRectGetMaxY(_leftTableView.frame)-2, self.frame.size.width, kButtomImageViewHeight);
        } completion:^(BOOL finished) {
...
- (void)animateTitle:(CATextLayer *)title show:(BOOL)show complete:(void(^)())complete {
    ...
    if ([title.string isEqualToString:@"筛选"] && self.isSelect) {
        title.foregroundColor = _textSelectedColor.CGColor;
    }else if ([title.string isEqualToString:@"筛选"] &&!self.isSelect) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.currentIndex == 3) {
        [cell.contentView addSubview:[self createrBackView]];
    }else{
        //NSAssert(_dataSource != nil, @"menu's datasource shouldn't be nil");
...
- (TagSearchView *)createrBackView{
    _titleArray = @[@"周末",@"最近1周",@"最近1月",@"最近3月"];
    _moneyArray = @[@"免费",@"付费"];
    
    if (self.tagView == nil) {
        self.tagView = [[TagSearchView alloc]initWithFrame:CGRectMake(0, 0, WIDTH , 340)];
    }
    [self.tagView createrTagSearchViewCity:_titleArray tagArray:_moneyArray];
    self.tagView.tagSearchViewDelegate = self;
    return self.tagView;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        NSLog(@"%@, = _currentSelectedMenudIndex  = %ld,row = %ld",self.currentIndexPath ,_currentSelectedMenudIndex,indexPath.row);
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    UIButton *btn = [self viewWithTag:100 + 0];
    
    CGFloat wideth1 = [NSHelper widthOfString:[NSString stringWithFormat:@"%@",title.string] font:[UIFont systemFontOfSize:14] height:14];
    if (wideth1 >= 50) {
        wideth1 =  16;
    } else if(wideth1 >= 30){
    ...
    CGFloat separatorLineInterval = self.frame.size.width / _numOfMenu;
    btn.frame = CGRectMake((0 + 1)*separatorLineInterval - wideth1 , self.frame.size.height / 2 - 3, 10, 10);
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        self.show = NO;
    }];
    
}
- (void)backgroundPackUpMenu{
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        self.show = NO;
    }];
}
- (void)gotoMakeSureTagSearchTime:(NSInteger)time  feerMoney:(NSInteger)feerMoney{
    UIButton *btn = [self viewWithTag:103];
    if (time == 0 && feerMoney == 0) {
        self.isSelect = NO;
        
        [btn setImage:kImageWithName(@"icon_barrow_grey") forState:UIControlStateNormal];
        if (self.finishedChangeBlock) {
            self.finishedChangeBlock(time-1,feerMoney - 1);
        }
    }else{
        self.isSelect = YES;
        [btn setImage:kImageWithName(@"icon_barrow_red") forState:UIControlStateNormal];
        if (self.finishedChangeBlock) {
            self.finishedChangeBlock(time-1,feerMoney - 1);
        }
    }
